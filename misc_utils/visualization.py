...
def visualize_detections(detections_dict, outfile="out.png"):
    colors = plt.cm.hsv(np.linspace(0, 1, 21)).tolist()
    plt.clf()
    plt.figure(figsize=(9 * len(detections_dict), 9))
    for pid, title in enumerate(detections_dict.keys()):
        ...
        for box in detections:
            ...
            xmax = min(
                int(box[-2] * input_img.shape[1] / model_img_size[1]),
                input_img.shape[1],
            )
            ymax = min(
                int(box[-1] * input_img.shape[0] / model_img_size[0]),
                input_img.shape[0],
            )
